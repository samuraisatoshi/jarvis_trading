...
# # import pandas_ta as ta (not needed, RSI calculated manually)
from loguru import logger


class FibonacciGoldenZoneStrategy:
    """
    ...
    def calculate_fibonacci_levels(
        ...
        if is_uptrend:
            ...
            levels = {
                ...
                '0.500': high - (diff * 0.500),  # Golden Zone start
                '0.618': high - (diff * 0.618),  # Golden Zone end
                '0.786': high - (diff * 0.786),  # Stop loss level
                '1.000': low,
                '1.618': high + (diff * 0.618),  # Target 1
                '2.618': high + (diff * 1.618),  # Target 2
            }
        else:
            ...
            levels = {
                ...
                '0.500': low + (diff * 0.500),  # Golden Zone start
                '0.618': low + (diff * 0.618),  # Golden Zone end
                '0.786': low + (diff * 0.786),  # Stop loss level
                '1.000': high,
                '1.618': low - (diff * 0.618),  # Target 1
                '2.618': low - (diff * 1.618),  # Target 2
            }
        ...
        logger.debug(
            ...
            f"  High: ${high:.2f}\n"
            f"  Low: ${low:.2f}\n"
            f"  Golden Zone: ${levels['0.618']:.2f} - ${levels['0.500']:.2f}\n"
            f"  Stop Loss: ${levels['0.786']:.2f}\n"
            f"  Target 1: ${levels['1.618']:.2f}\n"
            f"  Target 2: ${levels['2.618']:.2f}"
        )

        return levels

    def is_in_golden_zone(
        self, price: float, fib_levels: Dict[str, float], trend: str
    ) -> bool:
        """
        ...
        Args:
            price: Current price
            fib_levels: Dict of Fibonacci levels from calculate_fibonacci_levels()
        ...
        if trend == 'UPTREND':
            ...
            in_zone = fib_levels['0.618'] <= price <= fib_levels['0.500']
        elif trend == 'DOWNTREND':
            # Golden Zone: 50% <= price <= 61.8%
            in_zone = fib_levels['0.500'] <= price <= fib_levels['0.618']
        else:
            in_zone = False
        ...
        if in_zone:
            ...
            logger.debug(
                ...
                f"${fib_levels['0.618']:.2f} - ${fib_levels['0.500']:.2f}"
            )
    ...
    def generate_signal(self, df: pd.DataFrame) -> Dict:
        ...
        Returns:
            Dict with signal information:
            {
                ...
                'stop_loss': float (if BUY/SELL),
                'take_profit_1': float (if BUY/SELL),
                'take_profit_2': float (if BUY/SELL),
                'confirmations': list (if BUY/SELL),
                'fib_levels': dict (if BUY/SELL),
                'current_price': float,
                'golden_zone': str (if HOLD)
            }
        """
        try:
            # 1. Identify trend
            ...
            if not self.is_in_golden_zone(current_price, fib_levels, trend):
                golden_zone_str = f"${fib_levels['0.618']:.2f} - ${fib_levels['0.500']:.2f}"
                return {
                    'action': 'HOLD',
            ...
            if len(confirmations) < 2:
                return {
                    ...
                    'trend': trend,
                    'current_price': current_price,
                    'golden_zone': f"${fib_levels['0.618']:.2f} - ${fib_levels['0.500']:.2f}",
            ...
            if trend == 'UPTREND':
                ...
                if len(bullish_confirmations) >= 2:
                    return {
                        ...
                        'take_profit_1': fib_levels['1.618'],
                        'take_profit_2': fib_levels['2.618'],
